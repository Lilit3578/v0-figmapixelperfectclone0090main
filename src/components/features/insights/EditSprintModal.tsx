...
export const EditSprintModal: React.FC<EditSprintModalProps> = ({ sprint, onClose }) => {
  ...
  const [duration, setDuration] = useState(formatDuration(sprint.duration_seconds));
    ...
    <>
      ...
      >
        ...
        >
          <div className="flex justify-between items-center mb-6">
            <h2 
              className="text-[#1F1F1F] text-xl font-normal"
              style={{ 
            ...
            <button
              onClick={onClose}
              className="text-[rgba(31,31,31,0.64)] hover:text-[#1F1F1F] text-xl"
            >
              ×
            </button>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <textarea
                ...
                className="w-full px-4 py-2 bg-white border border-[rgba(31,31,31,0.12)] rounded resize-none"
                rows={3}
                style={{ 
                  fontFamily: 'Be Vietnam Pro',
                  fontSize: '14px',
      ...
      {showDeleteNotification && (
        <div className="fixed bottom-6 right-6 bg-[#1F1F1F] text-white px-6 py-3 rounded-lg shadow-lg flex items-center gap-3 z-50">
          ...
          >
            sprint-{sprint.id.slice(-2)} has been deleted
          </span>
          <button
            onClick={() => setShowDeleteNotification(false)}
            className="bg-white text-[#1F1F1F] px-3 py-1 rounded text-sm font-medium"
            style={{ 
          ...
          <button
            onClick={() => setShowDeleteNotification(false)}
            className="text-white hover:text-[rgba(255,255,255,0.8)]"
          >
            ×
          </button>
        </div>
