...
export const SprintsTab: React.FC<SprintsTabProps> = ({ sprints }) => {
  ...
  const formatSprintId = (id: string) => {
    return `SPRINT-${id.slice(-2).toUpperCase()}`;
  };
  ...
  if (sprints.length === 0) {
    return (
      <div style={{alignSelf: 'stretch', flex: '1 1 0', overflow: 'hidden', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', display: 'flex'}}>
        <div style={{color: 'rgba(31, 31, 31, 0.64)', fontSize: 14, fontFamily: 'Be Vietnam Pro', fontWeight: '400', lineHeight: 23.80, wordWrap: 'break-word'}}>
          No sprints found for this period
        </div>
      </div>
    );
  }
        ...
        {sprints.map((sprint) => (
          ...
          >
            ...
            <div style={{
              ...
              {formatDuration(sprint.duration_seconds)}
            </div>
            <div style={{
              width: 120, 
              color: '#1F1F1F', 
              ...
              {format(new Date(sprint.completed_at), 'EEE dd/MM')}
            </div>
            <div style={{
              flex: '1 1 0', 
              color: '#1F1F1F', 
