...
const TimerDisplay: React.FC<TimerDisplayProps> = ({ formattedTime }) => {
  ...
  // Use darker color when timer is active, lighter when idle
  const textColor = state === 'idle' ? 'rgba(31, 31, 31, 0.24)' : '#1F1F1F';
  
  // Show custom input in main timer area when in custom mode and input is active
  ...
  return (
    <div className="flex flex-col justify-center items-center w-full min-h-[200px] max-h-[400px]">
      <time 
        className="text-center font-normal leading-none tracking-[-0.02em] font-mono text-[clamp(60px,12vw,240px)] max-w-full"
        style={{ 
          fontFamily: 'Instrument Serif, serif',
          color: textColor
        }}
